    ...
    switch (action.type) {
        ...
        case GET_COUNTRY:
          const selectedCountry = state.countries.find(country => country.id == action.id);
            return Object.assign({}, state, {selectedCountry}); // zwraca wartość w nowym obiekcie

        case SEARCH_COUNTRIES:
          const foundCountries = state.countries.filter(country => country.name.toLowerCase().includes(action.searchText.toLowerCase()));
          // includes - czy w substringu znajduje się poszukiwana fraza
          // filter - zapisanie do foundCountries tylko elementów z poszukiwaną frazą
